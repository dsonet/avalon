...
(function(window) {
	 ...
	 *                    全局变量及方法                                  *
	 **********************************************************************/
	var EXPOSE = LEGACY ? new Date - 0 : Date.now()
	var SUBSCRIBES = "$" + EXPOSE
	var undefined
	var ex
	var document = window.document
	var stopRepeatAssign = false
	var STR_LIST_RE = /[^, ]+/g //切割字符串为一个个小块，以空格或逗号分开它们，结合replace实现字符串的forEach
	var NATIVE_CODE_RE = /\[native code\]/  //判定是否原生函数
	var COMPLEX_TYPE_RE = /^(?:object|array)$/
	var SVG_ELEMENT_RE = /^\[object SVG\w*Element\]$/
	var WINDOW_RE = /^\[object (?:Window|DOMWindow|global)\]$/
	var oProp = Object.prototype
	var oHasOwn = oProp.hasOwnProperty
	var serialize = oProp.toString
	var aProp = Array.prototype
	var aSlice = aProp.slice
	var currentComputeFn //方便访问器收集依赖
	var isW3C = window.dispatchEvent
	var isIE = window.VBArray
	var root = document.documentElement
	var head = LEGACY ? document.getElementsByTagName("head")[0] : document.head //HEAD元素
	var hyperspace = document.createDocumentFragment()
	var cinerator = document.createElement("div")
	var nativeClassType = {}
	"Boolean Number String Function Array Date RegExp Object Error".replace(STR_LIST_RE, function(name) {
		nativeClassType["[object " + name + "]"] = name.toLowerCase()
	})
	...
	if (DEBUG) {
		...
		if (LEGACY && !log) {
			log = {};
			"trace,debug,info,warn,error".replace(/\w+/g, function(n) {
				log[n] = noop
			})
		}
		else {
			"trace,debug,info,warn,error".replace(/\w+/g, function(n) {
				log[n] =console[n] ? function() {
	 ...
	 *                 avalon的静态方法定义区                              *
	 **********************************************************************/
	var avalon = window.avalon = function(el) { //创建jQuery式的无new 实例化结构
		return new avalon.init(el)
	}
	avalon.mode = LEGACY ? 'legacy' : ES5 ? 'es5' : ES6 ? 'es6' : 'unknow'
	...
	avalon.type = function(obj) {//取得目标的类型
		...
		return typeof obj === "object" || typeof obj === "function" ?
			nativeClassType[serialize.call(obj)] || "object" :
			typeof obj
	}
	var FUNCTION_RE = /^\s*\bfunction\b/
	var isFunction = LEGACY && typeof alert === "object" ? function(fn) {
		try {
			return FUNCTION_RE.test(fn + "")
		} catch (e) {
			return false
		}
	} : function(fn) {
	...
	avalon.isWindow = function(obj) {
		return WINDOW_RE.test(serialize.call(obj))
	}
	if (LEGACY) {
	...
	avalon.isPlainObject = function(obj) {
		return !!obj && typeof obj === "object" && Object.getPrototypeOf(obj) === oProp
	}
	if (LEGACY && !NATIVE_CODE_RE.test(Object.getPrototypeOf)) {
		avalon.isPlainObject = function(obj) {
	...
	var ALPHANUMERIC_RE = /\w/g;
	avalon.mix({
		rword: STR_LIST_RE,
		STR_LIST_RE: STR_LIST_RE,
		SUBSCRIBES: SUBSCRIBES,
		version: 1.35,
		ui: {},
		log: DEBUG ? log : noop,
		slice: LEGACY && isW3C || !LEGACY ? function(nodes, start, end) {
		...
		bind: function(el, type, fn, phase) {
			...
			if (typeof hook === "object") {
				type = hook.type
				if (hook.handler) {
					fn = hook.handler(el, fn)
				}
			}
			if (LEGACY) {
				var callback = isW3C ? fn : function(e) {
		...
		css: function(node, name, value) {
			...
			if (value === undefined || typeof value === "boolean") { //获取样式
		...
		getWidgetData: function(elem, prefix) {
			...
			for (var i in raw) {
				if (i.indexOf(prefix) === 0) {
					result[i.replace(prefix, "").replace(ALPHANUMERIC_RE, lowercase)] = raw[i]
				}
			}
			return result
		},
			...
			if (+n === n && !(n % 1) && n >= 0) { //检测length属性是否为非负整数
				try {
					if ({}.propertyIsEnumerable.call(obj, "length") === false) { //如果是原生对象
						return Array.isArray(obj) || FUNCTION_RE.test(obj.item || obj.callee)
					}
					return true
	...
	function pCallback(resolve) {
		resolve()
	}

	/*视浏览器情况采用最快的异步回调(在avalon.ready里，还有一个分支，用于处理IE6-9)*/
	avalon.nextTick = ES6 ? function(callback) {
		new Promise(pCallback).then(callback)
	} : window.setImmediate ? setImmediate.bind(window) : function(callback) {
		setTimeout(callback, 0) //IE10-11 or isW3C
	}
	...
	var vmRegistry = avalon.vmodels = {}//所有vmodel都储存在这里
	avalon.define = function(id, factory) {
		var $id = id.$id || id
		if (!$id) {
			DEBUG && log.error("vm必须指定$id")
		}
		if (vmRegistry[id]) {
			DEBUG && log.warn($id + " 已经存在于avalon.vmodels中")
		}
		if (typeof id === "object") {
		...
		model.$id = $id
		return vmRegistry[$id] = model
	}

	function modelFactory(scope, model) {
		if (Array.isArray(scope)) {
			//原数组的作为新生成的监控数组的$model而存在
		...
		var vmodel = ES6 ? scope : {} //要返回的对象
		var accessors = {} //监控属性
		var i, name, fn
		if (ES6) {
			if (!Array.isArray(vmodel.$skipArray)) {
				vmodel.$skipArray = []
			}
			vmodel.$skipArray.opposite = arguments[2] || {}
			avalon.each(vmodel, function(key, val) {
				if (!isObservable(key, val, vmodel.$skipArray)) {
					return //过滤所有非监控属性
				}
				var valueType = avalon.type(val)
				if (valueType === "object" && typeof val.get === "function" && Object.keys(val).length <= 2) {
					defineProperty(vmodel, key, {
						enumerable: true,
						configurable: true,
			...
			var clone = function(o){
				if(typeof o === 'object') {
					...
					if(Array.isArray(o)) {
						...
						ret = {}
						for(var n in o){
							if(SKIP_PROPERTIES.indexOf(n) === -1) {
								ret[n] = clone(o[n])
							}
						}
					}
			...
			if (Array.isArray(skipArray)) {
				for (i = 0; name = skipArray[i++];) {
					if (typeof name !== "string") {
						DEBUG && log.warn("$skipArray[" + name + "] must be a string")
					}
					normalProperties[name] = true
				}
			}
			for (i in scope) {
				accessorFactory(i, scope[i], model, normalProperties, accessors, computedProperties, watchProperties)
			}
			vmodel = defineProperties(vmodel, descriptorFactory(accessors)) //生成一个空的ViewModel
		...
		vmodel.$events = ES6 && vmodel.$events || {}
		vmodel.$id = generateID()
		vmodel.$accessors = accessors
		vmodel[SUBSCRIBES] = []
		for (i in EventManager) {
			fn = EventManager [i]
			if (LEGACY && !isW3C) { //在IE6-8下，VB对象的方法里的this并不指向自身，需要用bind处理一下
				fn = fn.bind(vmodel)
			}
			vmodel[i] = fn
		}
		if (LEGACY) {
			...
			defineProperty(vmodel, "hasOwnProperty", {
				value: function(name) {
					return name in vmodel.$model
				},
				configurable: true
	...
	//一些不需要被监听的属性
	var SKIP_PROPERTIES = ("$id,$watch,$unwatch,$fire,$events,$model,$skipArray,$accessors," + SUBSCRIBES).match(STR_LIST_RE)
	//比较两个值是否相等https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is
	...
	if (ES6) {
		var observeCallback = avalon.observeCallback = function(changes) {
			changes.forEach(function(change) {
				if (change.type === "update") {
					...
					notifySubscribers(host.$accessors)
				}
			})
		}
		...
		function isObservable(name, value, $skipArray) {
			return !(typeof value === "function"
				...
				|| name.charAt(0) === "$" && !$skipArray.opposite[name])
		...
		var descriptorFactory = isW3C ? function(obj) {
			var descriptors = {}
			for (var i in obj) {
				descriptors[i] = {
					get: obj[i],
		...
		function accessorFactory(name, val, model, normalProperties, accessors, computedProperties, watchProperties) {
			model[name] = val
			// 如果是元素节点 或者 在全局的SKIP_PROPERTIES里 或者在当前的$skipArray里
	...
	//应用于第2种accessor
	function updateVModel(parent, value, valueType) {
		//a为原来的VM， b为新数组或新对象
		if (valueType === "array") {
			...
			var bb = value.concat()
			parent.clear()
			parent.push.apply(parent, bb)
			return parent
		}
		else {
			var iterators = parent[SUBSCRIBES] || []
			if (withProxyPool[parent.$id]) {
				withProxyCount--
				delete withProxyPool[parent.$id]
			}
	...
	if (LEGACY) {
		...
		if (!defineProperties && isIE) {
			...
			var VBMediator = function(accessors, name, value) {
			...
			defineProperties = function(name, accessors, properties) {
				...
				return window[realClassName + "Factory"](accessors, VBMediator) //得到其产品
			}
		}
		/*********************************************************************
		 *                         javascript 底层补丁                       *
	...
	if (!ES6 && SVG && window.SVGElement) {
		var SVG_NS = "http://www.w3.org/2000/svg"
		var svg = document.createElementNS(SVG_NS, "svg")
		svg.innerHTML = '<circle cx="50" cy="50" r="40" fill="yellow" />'
		if (!SVG_ELEMENT_RE.test(svg.firstChild)) {// #409
			function enumerateNode(node, targetNode) {
				if (node && node.childNodes) {
					var nodes = node.childNodes
					for (var i = 0, el; el = nodes[i++];) {
						if (el.tagName) {
							var svg = document.createElementNS(SVG_NS,
								el.tagName.toLowerCase())
							aProp.forEach.call(el.attributes, function(attr) {
			...
			defineProperties(SVGElement.prototype, {
				...
				"innerHTML": {
					...
					configurable: true,
					get: function() {
						return  this.outerHTML.replace(new RegExp("<" + this.nodeName + '\\b(?:(["\'])[^"]*?(\\1)|[^>])*>', "i"), "")
							.replace(new RegExp("<\/" + this.nodeName + ">$", "i"), "")
					},
					set: function(html) {
						avalon.clearHTML(this)
						enumerateNode(avalon.parseHTML(html), this)
					}
				}
			})
		}
	...
	var openTag, closeTag, exprRe, exprsRe, exprBindRe, REGEXP_RE = /[-.*+?^${}()|[\]\/\\]/g

	function escapeRegExp(target) {
		//http://stevenlevithan.com/regex/xregexp/
		//将字符串安全格式化为正则表达式的源码
		return (target + "").replace(REGEXP_RE, "\\$&")
	}
	...
	function interpolate(array) {
		...
		if (openTag === closeTag) {
			...
			if (cinerator.innerHTML !== test && cinerator.innerHTML.indexOf("&lt;") >= 0) {
				throw new SyntaxError(test + "定界符不合法")
			}
			cinerator.innerHTML = ""
		}
		var o = escapeRegExp(openTag),
			c = escapeRegExp(closeTag)
		exprRe = new RegExp(o + "(.*?)" + c)
		exprsRe = new RegExp(o + "(.*?)" + c, "g")
		exprBindRe = new RegExp(o + ".*?" + c + "|\\sms-")
	}
	...
	if (LEGACY) {
		var ClassListMethods = {
			toString: function() {
				...
				return (typeof cls === "string" ? cls : cls.baseVal).split(WHITESPACE_RE).join(" ")
			},
		...
		function ClassList(node) {
			if (!("classList" in node)) {
				...
				for (var k in ClassListMethods) {
					node.classList[k] = ClassListMethods[k]
				}
			}
	...
	"add,remove".replace(STR_LIST_RE, function(method) {
		avalon.fn[method + "Class"] = function(cls) {
			var el = this[0]
			//https://developer.mozilla.org/zh-CN/docs/Mozilla/Firefox/Releases/26
			if (cls && typeof cls === "string" && el && el.nodeType === 1) {
				cls.replace(/\S+/g, function(c) {
					LEGACY ? ClassList(el)[method](c) : el.classList[method](c)
				})
			}
			return this
		}
	...
	avalon.fn.mix({
		hasClass: function(cls) {
			var el = this[0] || {}
			//IE10+, chrome8+, firefox3.6+, safari5.1+,opera11.5+支持classList,chrome24+,firefox26+支持classList2.0
			return el.nodeType === 1 && (LEGACY ? ClassList(el).contains(cls) : el.classList.contains(cls))
		},
		toggleClass: function(value, stateVal) {
			var className, i = 0
			var classNames = value.split(WHITESPACE_RE)
	...
	if (!ES6) {
		avalon.fn.data = function(name, value) {
			...
			switch (arguments.length) {
				...
				case 0:
					var ret = {}
					aProp.forEach.call(this[0].attributes, function(attr) {
						if (attr) {
							name = attr.name
	...
	function parseData(data) {
		try {
			data = data === "true" ? true :
					data === "false" ? false :
					data === "null" ? null : +data + "" === data ? +data : BRACE_RE.test(data) ? avalon.parseJSON(data) : data
		} catch (ex) {
		}
		return data
	}

	var BRACE_RE = /(?:\{[\s\S]*\}|\[[\s\S]*\])$/
	if (LEGACY) {
		var VALID_CHARS_RE = /^[\],:{}\s]*$/,
			VALID_BRACES_RE = /(?:^|:|,)(?:\s*\[)+/g,
			VALID_ESCAPE_RE = /\\(?:["\\\/bfnrt]|u[\da-fA-F]{4})/g,
			VALID_TOKENS_RE = /"[^"\\\r\n]*"|true|false|null|-?(?:\d+\.|)\d+(?:[eE][+-]?\d+|)/g
		avalon.parseJSON = window.JSON ? JSON.parse : function(data) {
			if (typeof data === "string") {
				data = data.trim();
				if (data && VALID_CHARS_RE.test(data.replace(VALID_ESCAPE_RE, "@")
					.replace(VALID_TOKENS_RE, "]")
					.replace(VALID_BRACES_RE, ""))) {
	...
	if (LEGACY && window.getComputedStyle || !LEGACY) {
		cssHooks["@:get"] = function(node, name) {
			if (!node || !node.style) {
				log.error("getComputedStyle要求传入一个节点 ", node)
			}
		...
		cssHooks["opacity:set"] = function(node, name, value) {
			...
			style.filter = (ALPHA_RE.test(filter) ?
					filter.replace(ALPHA_RE, opacity) :
					filter + " " + opacity).trim()
			if (!style.filter) {
		...
		cssHooks["opacity:get"] = function(node) {
			...
			return (op / 100) + "" //string 确保返回的是字符串
		}
	}

	"top,left".replace(STR_LIST_RE, function(name) {
		cssHooks[name + ":get"] = function(node) {
			var computed = cssHooks["@:get"](node, name)
			return PX_UNIT_RE.test(computed) ? computed :
	...
	"Width,Height".replace(STR_LIST_RE, function(name) {//fix 481
		...
		avalon.fn["inner" + name] = function() {
			return cssHooks[method + ":get"](this[0], undefined, -2)
		}
		avalon.fn["outer" + name] = function(includeMargin) {
			return cssHooks[method + ":get"](this[0], undefined, includeMargin === true ? 2 : 0)
		}
	})
	...
	var TAG_OPTION_RE = /^<option(?:\s+\w+(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+))?)*\s+value[\s=]/i
	var valHooks = {
		"option:get": function(node) {
			//在IE11及isW3C，如果没有指定value，那么node.value默认为node.text（有trim操作），但IE9-10则是取innerHTML(没trim操作)
			if (node.hasAttribute) {
				return node.hasAttribute("value") ? node.value : node.text.trim()
			}
			//specified并不可靠，因此通过分析outerHTML判定用户有没有显示定义value
			return TAG_OPTION_RE.test(node.outerHTML) ? node.value : node.text
		},
	...
	//取得其tagName
		...
		rcreate = isW3C ? /[^\d\D]/ : /(<(?:script|link|style|meta|noscript))/ig,
	...
	var tagHooks = {
		...
		_default: isW3C ? [0, ""] : [1, "X<div>"] //div可以不用闭合
	}
	tagHooks.optgroup = tagHooks.option
	tagHooks.tbody = tagHooks.tfoot = tagHooks.colgroup = tagHooks.caption = tagHooks.thead
	tagHooks.th = tagHooks.td
	if(SVG) {
		tagHooks.circle = tagHooks.ellipse = tagHooks.line = tagHooks.path = tagHooks.polygon = tagHooks.polyline = tagHooks.rect = tagHooks.text
	}
		...
		if (els.length) { //使用innerHTML生成的script节点不会发出请求与执行text属性
			for (i = 0; el = els[i++];) {
				if (!el.type || scriptTypes[el.type]) { //如果script节点的MIME能让其执行脚本
					...
					for (var k = 0, attr; attr = el.attributes[k++];) {
	...
	avalon.innerHTML = function(node, html) {
		if (LEGACY && !isW3C && (!rcreate.test(html) && !rnest.test(html))) {
			try {
				node.innerHTML = html
				return
			} catch (ex) {
	...
	var AVALON_CTRL_RE = /(\w+)\[(avalonctrl)="(\S+)"\]/
	var findNode = document.querySelector ? function(str) {
		return  document.querySelector(str)
	} : function(str) {
		var match = str.match(AVALON_CTRL_RE)
	 ...
	 *                           依赖调度系统                             *
	 **********************************************************************/
	var DUPLEX_ON_RE = /^(duplex|on)$/

	function registerSubscriber(data) {
		!ES6 && (currentComputeFn = data) //暴光此函数,方便collectSubscribers收集
		avalon.openComputedCollect = true
		var fn = data.evaluator
		if (fn) { //如果是求值函数
			try {
				var c = DUPLEX_ON_RE.test(data.type) ? data : fn.apply(0, data.args)
		...
		!ES6 && (currentComputeFn = null)
	}

	function collectSubscribers(accessor) { //收集依赖于这个访问器的订阅者
		if (currentComputeFn) {
			var list = accessor[SUBSCRIBES]
			if (list && avalon.Array.ensure(list, currentComputeFn) && currentComputeFn.element) {
				setTimeout(function() {
	...
	function scanTag(elem, vmodels) {
		...
		if (LEGACY) {
			...
			if (!elem.getAttributeNode) {
				DEBUG && log.warn(elem.tagName + " no getAttributeNode method")
				return
			}
		}
		if (typeof a === "string") {
		...
		var node = b || c
		if (node) {
			var vm = vmRegistry[node.value]
			if (!vm) {
				return
			}
			//ms-important不包含父VM，ms-controller相反
			vmodels = node === b ? [vm] : [vm].concat(vmodels)
			var name = node.name;
			elem.removeAttribute(name) //removeAttributeNode不会刷新[ms-controller]样式规则
			LEGACY ? avalon(elem).removeClass(node.name) : elem.classList.remove(node.name)
			elem.setAttribute("avalonctrl", node.value)
			vm.$events.expr = elem.tagName + '[avalonctrl="' + node.value + '"]'
		}
	...
	function scanAttr(elem, vmodels) {
		...
		var attributes = LEGACY && typeof getAttributes !== 'undefined' ? getAttributes(elem) : avalon.slice(elem.attributes)
		var bindings = [],
			msData = {},
			match
		...
		if (msData["ms-checked"] && msData["ms-duplex"]) {
			DEBUG && log.warn("一个元素上不能同时定义ms-checked与ms-duplex")
		}
		...
		switch (firstBinding.type) {
			...
			default:
				executeBindings(bindings, vmodels)
				if (!stopScan[elem.tagName] && exprBindRe.test(elem.innerHTML.replace(rlt, "<").replace(rgt, ">"))) {
		...
		do{
			...
			if (value) { //处理{{ }}插值表达式
				...
				tokens.push({
					value: value,
					expr: true,
					filters: leach.length ? leach : undefined
				})
			}
	...
	if (!LEGACY && typeof Set === "function" && (new Set([1, 2, 1])).size === 2) {
		uniqSet = function(arr) {//重写uniqSet
			var ret = []
	...
	//取得求值函数及其传参
	var DUPLEX_RE = /\w\[.*\]|\w\.\w/
	var PROXY_RE = /(\$proxy\$[a-z]+)\d+$/

	function getExprId(el) {
		return el.$id.replace(PROXY_RE, "$1")
	}

	function parseExpr(code, scopes, data) {
		var dataType = data.type
		var filters = data.filters ? data.filters.join("") : ""
		var exprId = scopes.map(getExprId) + code + dataType + filters
		var vars = getVariables(code).concat(),
			assigns = [],//收集赋值表达式
			names = [],
			args = [],//新生成的求值函数的传参 包括所有VM与avalon.filters对象
			prefix, i, name, sn, filterName
		//args 是一个对象数组， names 是将要生成的求值函数的参数
		scopes = uniqSet(scopes)
		if (vars.length) {
			for (i = 0, sn = scopes.length; i < sn; i++) {
				name = "vm" + EXPOSE + "_" + i
				names.push(name)
				args.push(scopes[i])
		...
		if (filters) { //文本绑定，双工绑定才有过滤器
			...
			for (i = 0; filterName = data.filters[i++];) {
				var start = filterName.indexOf("(")
				if (start !== -1) {
				...
				textBuffer.push(" if(filters", EXPOSE, ".", filterName, "){\n\ttry{\nret", EXPOSE,
					" = filters", EXPOSE, ".", filterName, "(ret", EXPOSE, filterArgs, ")\n\t}catch(ex){} \n}\n")
			}
			...
			var _body = "'use strict';\nreturn function(vvv){\n\t" +
				...
				";\n\tif(!arguments.length){\n\t\treturn " +
				code +
				"\n\t}\n\tswitch(typeof " + code + "){case 'number': vvv = Number(vvv);}" + (!DUPLEX_RE.test(code) ? vars.get : code) +
				"= vvv;\n} "
			try {
	...
	function parseExprProxy(code, scopes, data, tokens) {
		if (Array.isArray(tokens)) {
			code = tokens.map(getExprText).join(" + ")
		}
	...
	anomaly.replace(STR_LIST_RE, function(name) {
		propMap[name.toLowerCase()] = name
	})
	LEGACY ? head.insertBefore(avalon.parseHTML("<style>[ms-controller],[ms-important]{ display: none!important }</style>"), head.firstChild) : head.insertAdjacentHTML("afterBegin", "<style>[ms-controller],[ms-important]{ display: none!important }</style>")//避免IE6 base标签BUG
	var NOSCRIPT_RE = /<noscript.*?>(?:[\s\S]+?)<\/noscript>/img
	var NOSCRIPT_TEXT_RE = /<noscript.*?>([\s\S]+?)<\/noscript>/im

	var getXHR = function() {
		return LEGACY ? new (window.XMLHttpRequest || ActiveXObject)("Microsoft.XMLHTTP") : new XMLHttpRequest();
	}
	...
	var templateCache = avalon.templateCache = {}
	var ifSanctuary = document.createElement("div")
	...
	var boolMap = {}
	String("autofocus,autoplay,async,allowTransparency,checked,controls,declare,disabled,defer,defaultChecked,defaultSelected" +
		"contentEditable,isMap,loop,multiple,noHref,noResize,noShade,open,readOnly,selected")
		.replace(STR_LIST_RE, function(name) {
	...
	var bindingExecutors = avalon.bindingExecutors = {
		"attr": function(val, elem, data) {
			...
			if (method === "css") {
				...
				var toRemove = (val === false) || (val === null) || (val === undefined)
				if (LEGACY && !isW3C && propMap[attrName]) {//旧式IE下需要进行名字映射
				...
				if (data.param === "src") {
						...
						xhr[LEGACY ? 'onreadystatechange' : 'onload'] = function() {
							if (LEGACY && xhr.readyState === 4 || !LEGACY) {
								...
								if (s >= 200 && s < 300 || s === 304 || s === 1223) {
									scanTemplate(templateCache[val] = xhr.responseText)
								}
							}
						}
						xhr.open("GET", val, true)
						if (LEGACY && "withCredentials" in xhr || !LEGACY) {
					...
					if (el) {
						if (el.tagName === "NOSCRIPT" && !(el.innerHTML || el.fixIE78)) { //IE7-8 innerText,innerHTML都无法取得其内容，IE6能取得其innerHTML
							...
							var array = (xhr.responseText || "").match(NOSCRIPT_RE) || []
							for (var i = 0, n = array.length; i < n; i++) {
								var tag = noscripts[i]
								if (tag) { //IE6-8中noscript标签的innerHTML,innerText是只读的
									tag.style.display = "none" //http://haslayout.net/css/noscript-Ghost-Bug
									tag.fixIE78 = (array[i].match(NOSCRIPT_TEXT_RE) || ["", "&nbsp;"])[1]
								}
							}
						}
						avalon.nextTick(function() {
					...
					case "active":
						if (!data.hasBindEvent) { //确保只绑定一次
							...
							if (method === "active") {//在聚焦失焦中切换类名
								...
								activate = "mousedown"
								abandon = "mouseup"
								$elem.bind("mouseleave", removeClassFn)
							}
							$elem.bind(activate, function() {
								data.toggleClass && $elem.addClass(data.newClass)
							})
							$elem.bind(abandon, removeClassFn)
		...
		"repeat": function(method, pos, el) {
			if (method) {
				...
				var i, key, n, last, proxy, spans, lastFn
				switch (method) {
					case "add": //在pos位置后添加el数组（pos为数字，el为数组）
						spans = []
						lastFn = {}
						last = data.$repeat().length - 1
					...
					case "append": //将pos的键值对从el中取出（pos为一个普通对象，el为预先生成好的代理VM对象池）
						...
						for (i = 0; key = keys[i++];) {
							if (key !== "hasOwnProperty") {
								lastFn = shimController(data, transition, spans, el[key])
							}
						}
						lastFn.parent = parent
		...
		"if": function(val, elem, data) {
			var placeholder = data.placehoder
			if (val) { //插回DOM树
	...
	var bindingHandlers = avalon.bindingHandlers = {
		...
		"attr": function(data, vmodels) {
			...
			if (text.indexOf(openTag) > -1 && text.indexOf(closeTag) > 2) {
				simple = false
				if (exprRe.test(text) && RegExp.rightContext === "" && RegExp.leftContext === "") {
					simple = true
					text = RegExp.$1
				}
			}
		...
		"class": function(data, vmodels) {
			...
			if (!oldStyle || isFinite(oldStyle)) {
				data.param = "" //去掉数字
				var noExpr = text.replace(exprsRe, function(a) {
				...
				var hasExpr = exprRe.test(className) //比如ms-class="width{{w}}"的情况
				if (!hasExpr) {
		...
		"repeat": function(data, vmodels) {
			...
			data.$repeat = repeatItemEvaluator
			data.proxies = []
			var stop = true
			try {
				list = data.$repeat()
			...
			if (type === "repeat") {
				var startRepeat = document.createComment("ms-repeat-start")
				var endRepeat = document.createComment("ms-repeat-end")
			...
			if (arr.length > 1) {
				...
				var n = arr[0]
				for (i = 0; v = vmodels[i++];) {
					if (v && v.hasOwnProperty(n) && v[n][SUBSCRIBES]) {
						v[n][SUBSCRIBES].push(data)
						break
					}
				}
			}
			...
			for (i = 0; v = vmodels[i++];) {
				if (v.hasOwnProperty(prop1) && v.hasOwnProperty(prop2)) {
					data.$outer = v
					break
				}
			}
		...
		"if": function(data, vmodels) {
			...
			elem.removeAttribute(data.name)
			if (!data.placehoder) {
				data.msInDocument = data.placehoder = document.createComment("ms-if")
			}
			data.vmodels = vmodels
			parseExprProxy(data.value, vmodels, data)
		},
		...
		"widget": function(data, vmodels) {
			var args = data.value.match(STR_LIST_RE)
			var elem = data.element
			var widget = args[0]
			if (args[1] === "$" || !args[1]) {
	...
	"hover,active".replace(STR_LIST_RE, function(method) {
		bindingHandlers[method] = bindingHandlers["class"]
	})
	"with,each".replace(STR_LIST_RE, function(name) {
	...
	"title,alt,src,value,css,include,href".replace(STR_LIST_RE, function(name) {
	...
	try {
		var inputProp = HTMLInputElement.prototype
		Object.getOwnPropertyNames(inputProp)//故意引发IE6-8等浏览器报错
		var oldSetter = Object.getOwnPropertyDescriptor(inputProp, "value").set //屏蔽chrome, safari,opera
		defineProperty(inputProp, "value", {
			set: newSetter
		})
	} catch (ex) {
		launch = avalon.tick
	...
	if (document.onmousewheel === undefined) {
		...
		var fixWheelType = document.onwheel !== undefined ? "wheel" : "DOMMouseScroll"
		...
		eventHooks.mousewheel = {
			...
			handler: function(elem, fn) {
				return function(e) {
					...
					e.wheelDeltaX = 0
					if (LEGACY && defineProperty || !LEGACY) {
						defineProperty(e, "type", {
							value: "mousewheel"
						})
					}
					fn.call(elem, e)
	...
	var CollectionPrototype = {
		...
		push: function() {
			aProp.push.apply(this.$model, arguments)
		...
		unshift: function() {
			aProp.unshift.apply(this.$model, arguments)
	...
	"sort,reverse".replace(STR_LIST_RE, function(method) {
		CollectionPrototype[method] = function() {
			var aaa = this.$model,
				bbb = aaa.slice(0),
				sorted = false
			aProp[method].apply(aaa, arguments) //先移动model
	...
	function convert(val) {
		if (COMPLEX_TYPE_RE.test(avalon.type(val))) {
	...
	if (isIE) {
		if (VML) {
			var isVML = function(src) {
				var nodeName = src.nodeName
				return  nodeName.toLowerCase() === nodeName && src.scopeName && src.outerText === ""
			}
			var fixVML = function(node) {
	...
	function shimController(data, transition, spans, proxy) {
		...
		if (!data.fastRepeat) {
			span = document.createElement("msloop")
	...
	function createWithProxy(key, val, $outer) {
		...
		proxy.$id = ("$proxy$with" + Math.random()).replace(ZERO_RE, "")
		return proxy
	}
	...
	function getEachProxy(index, item, data, last) {
		...
		if (COMPLEX_TYPE_RE.test(avalon.type(item))) {
		...
		proxy.$id = ("$proxy$" + data.type + Math.random()).replace(ZERO_RE, "")
		return proxy
	}
	...
	if (FILTERS) {
		...
		var filters = avalon.filters = {
			...
			sanitize: !LEGACY && window.toStaticHTML ? toStaticHTML.bind(window) : function(str) {
				return str.replace(/<script[^>]*>([\S\s]*?)<\/script\s*>/gim, "").replace(/<\w+\b(?:("|')[^\1]*?\1|[^>])*>/ig, function(a) {
					if (/^<(a|img)\s/i.test(a)) {
						a = a.replace(/\s+(src|href)(?:=("javascript[^"]*"|'javascript[^']*'))?/ig, " $1=''")//移除javascript伪协议
					}
					return a.replace(/\s+(on[^=\s]+)(?:=("[^"]*"|'[^']*'|[^\s>]+))?/g, " ").replace(WHITESPACE_RE, " ")//移除onXXX事件
				})
			},
			escape: function(html) {
				...
				return String(html)
					.replace(/&/g, '&amp;')
					.replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, surrogate)
					.replace(/([^\#-~| |!])/g, noalphanumeric)
					.replace(/</g, '&lt;')
					.replace(/>/g, '&gt;')
			},
			currency: function(number, symbol) {
				symbol = symbol || "\uFFE5"
				return symbol + filters.number(number)
			},
		...
		function jsonStringToDate(string) {
			var match
			if (match = string.match(/^(\d{4})-?(\d\d)-?(\d\d)(?:T(\d\d)(?::?(\d\d)(?::?(\d\d)(?:\.(\d+))?)?)?(Z|([+-])(\d\d):?(\d\d))?)?$/)) {
				var date = new Date(0),
					tzHour = 0,
					tzMin = 0,
		...
		filters.date = function(date, format) {
			var locale = filters.date.locale,
				text = "",
				parts = [],
				fn, match
			...
			while (format) {
				match = dateFormatSplit.exec(format)
				if (match) {
	...
	if (AMD) {
		...
		var factories = [] //储存需要绑定ID与factory对应关系的模块（标准浏览器下，先parse的script节点会先onload）
		function getCurrentScript(base) {
			// 参考 https://github.com/samyk/jiagra/blob/master/jiagra.js
			var stack
			try {
				a.b.c() //强制报错,以便捕获e.stack
			} catch (ex) { //safari的错误对象只有line,sourceId,sourceURL
				stack = ex.stack
				if (LEGACY && !stack && window.opera) {
					//opera 9没有e.stack,但有e.Backtrace,但不能直接取得,需要对e对象转字符串进行抽取
					stack = (String(ex).match(/of linked script \S+/g) || []).join(" ")
				}
			}
			if (stack) {
			...
			for (var i = nodes.length, node; node = nodes[--i];) {
				if ((base || node.className === SUBSCRIBES) && node.readyState === "interactive") {
		...
		config.plugins.baseUrl = function(v) {
			baseUrl = v;
		}

		var baseUrl = getPath(getCurrentScript(true) || document.scripts[document.scripts.length - 1].src);
		baseUrl = config.baseUrl = baseUrl.slice(0, baseUrl.lastIndexOf("/") + 1);

		loaders.js = function(url, shim) {
			var id = getPath(url)
		...
		loaders.css = function(url) {
			...
			if (!document.getElementById(id)) {
				...
				node.href = url
				node.id = id
				LEGACY ? head.insertBefore(node, head.lastChild) : head.appendChild(node)
			}
		}
		loaders.css.ext = ".css"
		loaders.js.ext = ".js"

		loaders.text = function(url) {
			var xhr = getXHR()
			var id = url.replace(/[?#].*/, "")
			modules[id] = {}
			xhr[LEGACY ? 'onreadystatechange' : 'onload'] = function() {
		...
		if (!LEGACY && 'import' in document.createElement("link")) {
			loaders.text = function(url) {
				var id = url.replace(/[?#].*/, "")
				modules[id] = {}
		...
		function checkDependencies() {
			...
			loop: for (var i = loadings.length, id; id = loadings[--i];) {
				...
				if (obj.state !== 2) {
					...
					checkDependencies() //如果成功,则再执行一次,以防有些模块就差本模块没有安装好
				}
			}
		}

		function loadCallback(node, onError, fuckIE) {
			var id = getPath(node.src) //检测是否死链
			node.onerror = null
			LEGACY ? node.onreadystatechange = null : node.onload = null
		...
		function loadResource(url, parent, ret, shim) {
			//1. 特别处理mass|ready标识符
			...
			loader = loaders[loader || "js"] || noop
			//3. 转化为完整路径
			if (typeof config.shim[url] === "object") {
				shim = config.shim[url]
			}
			url = url.split('/');
			...
			for (var i = url.length, parentModule, parentPath; i > 0; i -= 1) {
				...
				parentPath = config.paths[parentModule];
				if (parentPath) {
			...
			//6. 缓存处理
			if (config.nocache) {
				ret += (ret.indexOf("?") === -1 ? "?" : "&") + EXPOSE
			}
			return loader(ret, shim)
		}

		function loadJS(url, id, callback) {
			//通过script节点加载目标模块
			var node = document.createElement("script")
			node.className = SUBSCRIBES //让getCurrentScript只处理类名为subscribers的script节点
			node[LEGACY && isW3C || !LEGACY ? "onload" : "onreadystatechange"] = function() {
				if (LEGACY && (isW3C || /loaded|complete/i.test(node.readyState)) || !LEGACY) {
					//mass Framework会在_loadCallback把它上面的回调清掉，尽可能释放回存，尽管DOM0事件写法在IE6下GC无望
					var factory = factories.pop()
					factory && factory.delay(id)
					if (callback) {
						callback()
					}
					if (loadCallback(node, false, LEGACY ? !isW3C : false)) {
						//DEBUG && log.debug("已成功加载 ", url)
					}
				}
			}
			node.onerror = function() {
				loadCallback(node, true)
			}
			node.src = url //插入到head的第一个节点前，防止IE6下head标签没闭合前使用appendChild抛错
			LEGACY ? head.insertBefore(node, head.lastChild) : head.appendChild(node)//chrome下第二个参数不能为null
			//DEBUG && log.debug("正准备加载 " + url) //更重要的是IE6下可以收窄getCurrentScript的寻找范围
		}

		var require = window.require = function(list, factory, parent) {
			// 用于检测它的依赖是否都为2
			var deps = {},
			// 用于保存依赖模块的返回值
				args = [],
			...
			String(list).replace(STR_LIST_RE, function(el) {
				var url = loadResource(el, parent || baseUrl)
				if (url) {
					dn++
			...
			avalon.mix(modules[id], {//创建一个对象,记录模块的加载情况与其他信息
				id: id,
				factory: factory,
				deps: deps,
				args: args,
		 ...
		 * @api public
		 */
		var define = window.define = function(id, deps, factory) { //模块名,依赖列表,模块本身
			...
			var name = _id || getPath(getCurrentScript())
			if (!modules[name] && _id) {
				modules[name] = {
					id: name,
					factory: factory,
				...
				_id && (modules[getPath(getCurrentScript())] = modules[name])
			}
			factory = args[1]
			factory.delay = function(d) {
				...
				var isCycle = true
				try {
					isCycle = modules[d] && checkCycle(modules[d].deps, d)
				} catch (ex) {
				}
				if (isCycle) {
					throw new Error(d + "模块与之前的模块存在循环依赖，请不要直接用script标签引入" + d + "模块")
				}
		...
		function fireFactory(id, deps, factory) {
			...
			var module = modules[id],
				ret = factory && factory.apply(window, array)
			module.state = 2
			if (ret !== undefined) {
				module.exports = ret
			}
	...
	function fireReady() {
		if (document.body) { //  在IE8 iframe中doScrollCheck可能不正确
			...
			fireReady = noop //隋性函数，防止IE9二次调用_checkDependencies
		}
	}
